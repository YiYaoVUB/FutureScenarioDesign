...
pd_reader = pd.read_csv( 'C:\Research2\Data_for_calculation\Final_calculation\Data_GDP_population_projection_' + scenario + '.csv')     # Projected GDP_per_capita data
data = np.array(pd_reader)
gdp_2015_2100 = data[:, 1:]

pd_reader = pd.read_csv('C:\Research2\Data_for_calculation\Final_calculation\goverance_projection_' + scenario + '.csv')        # Projected goverance data
data = np.array(pd_reader)
gov_2015_2100 = data[:, 1:]

pd_reader = pd.read_csv('C:\Research2\Data_for_calculation\Final_calculation\\urbanization_projection_' + scenario + '.csv')    # Projected urbanization data
data = np.array(pd_reader)
...
for i in range(len_country):
    try:
        ...
        gdp_hist_code = int(GDP_code_hist[country_code[i]]) - 1
        gov_hist_code = int(GOV_code_hist[country_code[i]]) - 1
        urb_hist_code = int(URB_code_hist[country_code[i]]) - 1

        gdp_proj_code = int(GDP_code_proj[country_code[i]]) - 1
        gov_proj_code = int(GOV_code_proj[country_code[i]]) - 1
        urb_proj_code = int(URB_code_proj[country_code[i]]) - 1
